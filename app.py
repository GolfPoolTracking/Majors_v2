...
else:import streamlit as st, pandas as pd, requests, re, datetime, smtplib, pytz, json, hashlib, altair as alt, time
import html, threading
from email.mime.text import MIMEText
from email.mime.multipart import MIMEMultipart
from email.utils import formataddr
...
def safe_parse_dt(dt_input, fallback_dt):
    """Safely parses datetimes from Supabase, handling ISO formats and missing seconds."""
    if not dt_input: return fallback_dt
    try:
        if isinstance(dt_input, datetime.datetime): return dt_input
        dt_str = str(dt_input).replace('T', ' ').split('.')[0].split('+')[0].strip()
        if len(dt_str) == 10:
            dt_str += " 05:00:00"
        elif len(dt_str) == 16:
            dt_str += ":00"
...
def calculate_leaderboard(t_id, t_name, t_start, par_override=0, dns_input="", valid_players=None, logo_url=None, is_admin_download=False, header_only=False, is_admin_view=False, hide_title=False, search_query=""):
    try:
        ...
        if t_status == 'completed' or (current_r == 4 and (is_r4_fully_done or last_group_htr >= 18)):
            ...
            parsed_start = safe_parse_dt(t_start, None)
            start_str = parsed_start.strftime('%d %b %y') if parsed_start else str(t_start)
            status_txt = f"Waiting for R1 to start... ⏳ ({start_str})" if t_start else "Waiting for R1 to start... ⏳"
...
if is_public:
    ...
    t_start_dt = safe_parse_dt(t_start, None)
    ...
    close_time = safe_parse_dt(db_close, default_close)
    reveal_time = safe_parse_dt(db_reveal, default_reveal)
    ...
    with st.sidebar:
        ...
        if t_id:
            ...
            base_start_dt = safe_parse_dt(t_start, datetime.datetime.now())
            default_close_dt = base_start_dt.replace(hour=5, minute=0, second=0)
            db_close_admin = fetch_close_time_from_db(t_id)
            current_close_dt = safe_parse_dt(db_close_admin, default_close_dt)
            ...
            default_reveal_dt = base_start_dt.replace(hour=5, minute=0, second=0)
            db_reveal_admin = fetch_reveal_time_from_db(t_id)
            current_reveal_dt = safe_parse_dt(db_reveal_admin, default_reveal_dt)
                    ...
                    if not added_players and not removed_players: st.success("✅ The field is currently stable. No changes detected.")
                        ...
                        if st.button("🚀 Send Alert Emails", type="primary"):
                            with st.spinner("Sending emails..."):
                                current_close_dt = safe_parse_dt(fetch_close_time_from_db(t_id), datetime.datetime.now())
                                try:
